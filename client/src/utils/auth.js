...
class AuthService {
    ...
    //checking if the token is valid still. The try block compares the token expiration date and the current date to see if it has past the expiration date. After which if it is less than, then the token is valid and passes as true. If expired then returns false.
    isTokenExpired(token) {
        try {
            ...
            //I have no idea what the auth does here. haha
        } catch (err) {auth 
        return false
     }
    }
